import { ArrowRight, Instagram, Link2, MapPin, PhoneCall, Mail } from "lucide-react";
import { motion, useScroll, useTransform } from "motion/react";
import { useState } from "react";

import logoUrl from "@/assets/dd-logo.png";
...
const products = [
  {
    name: "Lagos Nutty Traffic",
    label: "Signature",
    price: "₦1,800",
    description: "Caramel glaze, roasted groundnuts, sea salt.",
    image: lagosNutty,
  },
  {
    name: "Milo Madness",
    label: "Bold",
    price: "₦1,600",
    description: "Choc malt cream fill, dark choc glaze, Milo dust.",
    image: miloMadness,
  },
  {
    name: "Zobo Sweet Rush",
    label: "Fresh",
    price: "₦1,700",
    description: "Hibiscus zobo glaze, dried berries, coconut flakes.",
    image: zoboRush,
  },
  {
    name: "Northern Spice Street",
    label: "Spiced",
    price: "₦1,700",
    description: "Nutmeg and ginger dough with a rich coffee glaze.",
    image: northernSpice,
  },
];
    ...
    <div className="min-h-screen overflow-hidden bg-cream font-sans text-ink selection:bg-brand/25">
      ...
      <main id="top">
        <section className="mx-auto grid min-h-[calc(100vh-64px)] max-w-6xl items-center gap-12 px-5 pb-20 pt-12 sm:px-8 lg:grid-cols-[1.05fr_0.95fr] lg:pt-8">
          ...
          >
            <motion.p
              variants={rise}
              className="mb-6 inline-flex items-center gap-2 text-xs font-semibold uppercase text-accent"
            >
            ...
            >
              Street-level donuts,
              <br />
              <span className="text-brand">glazed with intent.</span>
            </motion.h1>
            <motion.p variants={rise} className="mt-6 max-w-md text-lg leading-relaxed text-ink/70">
              A small-batch pastry counter bringing premium local flavours to Benin City—baked
              fresh, full of character, no shortcuts.
            </motion.p>
            <motion.form
              variants={rise}
            ...
            >
              <input
                ...
                className="min-w-0 flex-1 rounded-full border border-ink/15 bg-surface px-5 py-3.5 text-sm outline-none transition-colors placeholder:text-ink/40 focus:border-accent"
              />
            ...
            <motion.p variants={rise} aria-live="polite" className="mt-3 text-xs text-ink/50">
              {submitted
                ? "You're on the list. We’ll see you at launch."
                : "Free to join. Your first launch-day donut is on us."}
            </motion.p>
          </motion.div>
          ...
          >
            <div className="aspect-[9/10] overflow-hidden rounded-2xl bg-sand">
              <motion.img
                ...
                src={donutHero}
                alt="A selection of glossy artisan donuts on a dark wooden table"
                width={1080}
                height={1200}
            ...
            <motion.div
              ...
              className="absolute -bottom-5 left-3 rounded-xl border border-ink/10 bg-cream px-5 py-4 shadow-soft sm:-left-4"
            >
            ...
            >
              <div>
                <p className="mb-3 text-xs font-semibold uppercase text-brand">
                  Naija Street Collection
                </p>
                <h2 className="font-display text-4xl font-semibold sm:text-5xl">The first four</h2>
              </div>
            ...
            <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
              {products.map((product, index) => (
                ...
                >
                  ...
                  <h3 className="mt-1 min-h-14 font-display text-2xl font-semibold leading-tight">
                    {product.name}
                  </h3>
                  <p className="mt-1.5 min-h-16 text-sm leading-relaxed text-cream/55">
                    {product.description}
                  </p>
        ...
        <section id="waitlist" className="mx-auto max-w-6xl px-5 py-20 sm:px-8">
          <motion.div
            ...
            className="grid items-center gap-8 rounded-3xl bg-accent px-7 py-12 text-cream sm:px-14 lg:grid-cols-2"
          >
            <div>
              <h2 className="font-display text-4xl font-semibold leading-tight">
                Be first on the counter.
              </h2>
              <p className="mt-4 max-w-sm leading-relaxed text-cream/75">
                Pick your flavours now, get served first on launch day, and enjoy a free donut.
              </p>
            </div>
            <div className="w-full max-w-md lg:justify-self-end">
              <form onSubmit={submitWaitlist} className="flex flex-col gap-3 sm:flex-row">
                <input
                  ...
                  className="min-w-0 flex-1 rounded-full border border-cream/25 bg-cream/10 px-5 py-3.5 text-sm outline-none placeholder:text-cream/45 focus:border-cream/60"
                />
                <motion.div whileHover={{ y: -3 }} whileTap={{ scale: 0.97 }}>
                  <Button type="submit" variant="light" size="large">
                    Reserve
                  </Button>
                </motion.div>
              </form>
              <p aria-live="polite" className="mt-3 text-xs text-cream/55">
                {submitted ? "Spot reserved. Thank you!" : "We’ll only send the good stuff."}
              </p>
            </div>
          </motion.div>
        </section>
      ...
      <footer id="contact" className="border-t border-ink/10">
        <div className="mx-auto flex max-w-6xl flex-col items-start justify-between gap-7 px-5 py-10 sm:px-8 md:flex-row md:items-center">
          <div>
            <a
              ...
              aria-label="Donut District home"
            >
              <img
                src={logoUrl}
                alt="Donut District logo"
            ...
            <p className="mt-2 flex items-center gap-2 text-sm text-ink/55">
              <MapPin size={15} /> Benin City, Edo State, Nigeria
            </p>
          </div>
          <div className="flex flex-wrap items-center gap-5 text-sm font-medium">
            <a
              href="https://instagram.com/donutdistricthq"
              target="_blank"
              rel="noreferrer"
            ...
            <a
              href="https://tiktok.com/@donutdistricthq"
              target="_blank"
              rel="noreferrer"
              className="nav-link inline-flex items-center gap-1.5"
            >
              <Link2 size={15} /> TikTok
            </a>
            <a href="tel:+2347014298844" className="nav-link inline-flex items-center gap-1.5">
              <PhoneCall size={15} /> 0701 429 8844
            </a>
            <a
              href="mailto:donutdistrictfood@gmail.com"
              className="nav-link inline-flex items-center gap-1.5"
            >
              <Mail size={15} /> Email
            </a>
        ...
        <p className="pb-8 text-center text-xs text-ink/35">
          © 2026 Donut District · Baked in Benin City
        </p>
      </footer>
    </div>
  );
